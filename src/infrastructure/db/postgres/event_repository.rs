...
use crate::{
    ...
    }, domain::event::{event::Event, event_data_value::EventDataValue, event_format::EventFormat},
};
...
impl CreateEventRepository for PostgresEventRepository {
    async fn create_event(&self, evt: Event, _: &EventFormat) -> Result<(), EventRepositoryError> {
        ...
        let event_data: HashMap<String, Value> = evt
            .payload
            .into_iter()
            .map(|(k, v)| (k, v.into()))
            .collect();
...
impl GetEventRepository for PostgresEventRepository {
    async fn get_events(
        ...
        let events: Result<Vec<Event>, EventRepositoryError> = rows
            .into_iter()
            .map(|row| {
                ...
                for (k, v) in payload_db.0 {
                    let val = EventDataValue::try_from(v).map_err(|_| {
                        EventRepositoryError::RepositoryError(format!(
