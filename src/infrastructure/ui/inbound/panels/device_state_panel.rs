...
use crate::{
    ...
    infrastructure::ui::inbound::{
        LoadingStatus, managers::device_state_manager::DeviceStateManager, widgets::gauge::gauge,
    },
};
...
pub fn display_device_state_panel<AO: AppOutbound + 'static>(
    ...
    match device_states_mtx.lock() {
        Ok(device_states_res) => {
            match device_states_res.clone() {
                Ok(device_states) => {
                                        ...
                                        egui::Frame::group(ui.style())
                                            ...
                                            .show(ui, |ui| {
                                                ui.vertical(|ui| {
                                                    ...
                                                    {
                                                        let device_state_values =
                                                            device_state.values.unwrap();
                                                        for (key, value) in device_state_values {
                                                            ui.label(format!("{}:", key.to_uppercase()));
                                                            if let EventDataValue::Number(num) =
                                                                value
                                                            {
                                                                gauge(
                                                                    ui,
                ...
                Err(LoadingStatus::NotStarted) => {
                    should_load = true;
                }
                Err(LoadingStatus::Failed(e)) => {
                    ui.label(format!("Failed to load device states : {}", e));
                }
            }
        }
        Err(_) => {
            device_states_mtx.clear_poison();
            should_repaint = true;
        }
    }
    if should_load {
