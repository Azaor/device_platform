...
use crate::{
    ...
    infrastructure::mqtt::mqtt_messages::{
        self, MqttActionEmittable, MqttActionType, MqttEventEmittable,
    },
};
...
impl MqttDeviceRepository {
    pub fn new(mqtt_client: AsyncClient, device_topic: &str) -> Self {
        return MqttDeviceRepository {
            mqtt_client,
            device_topic: device_topic.to_string(),
        };
    }
}
...
impl CreateDeviceRepository for MqttDeviceRepository {
    async fn create(&self, device: &Device) -> Result<(), DeviceRepositoryError> {
        let events_serialized: HashMap<String, MqttEventEmittable> = device
            .events()
            .iter()
            .map(|(k, v)| (k.to_string(), MqttEventEmittable::from(v)))
            .collect();
        let events = match serde_json::to_string(&events_serialized) {
            Ok(r) => r,
            Err(e) => return Err(DeviceRepositoryError::InternalError(e.to_string())),
        };
        let actions_serialized: HashMap<String, MqttActionEmittable> = device
            .actions()
            .iter()
            .map(|(k, v)| (k.to_string(), MqttActionEmittable::from(v)))
            .collect();
        let actions = match serde_json::to_string(&actions_serialized) {
            Ok(a) => a,
            Err(e) => return Err(DeviceRepositoryError::InternalError(e.to_string())),
        };
        ...
        let message =
            match mqtt_messages::payload_to_mqtt_message(mqtt_payload, MqttActionType::Create) {
                Ok(r) => r,
                Err(e) => {
        ...
        self.mqtt_client
            .publish(&self.device_topic, rumqttc::QoS::AtLeastOnce, true, message)
            .await
            .map_err(|e| {
...
impl DeleteDeviceRepository for MqttDeviceRepository {
    async fn delete_by_id(&self, id: Uuid) -> Result<(), DeviceRepositoryError> {
        let payload = mqtt_messages::DeleteDevicePayload { id: id.to_string() };
